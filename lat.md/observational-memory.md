...
- **System prompt** (pi 0.86.0 removed `AgentContext.systemPrompt`): each worker sets the legacy `systemPrompt` property always, *and* prepends the prompt as a leading `role: "system"` message in `context.messages` **only on hosts ≥0.86** — gated by [[src/om/agents/host-compat.ts]] `hostUsesInbandSystemMessage()` (compares the host's exported `VERSION`). The gate is mandatory: on hosts ≤0.85 their pi-ai `estimateMessageTokens` iterates in-band message content as content blocks, so a system message with *string* content crashes with `Cannot read properties of undefined (reading 'length')` inside `clampMaxTokensToContext` — a path every provider `streamSimple` calls, which exhausted all candidate models (the 2026-09-21 "observer failed: all model candidates exhausted" regression of the ungated dual mechanism). Hosts ≥0.86 ignore the property and read the message (replayed by `normalizeContext`/`getCurrentSystemPrompt`).
