...
export function registerCompactFailedHook(pi: ExtensionAPI, runtime: Runtime): void {
	...
	const onAny = pi.on as unknown as (event: string, handler: (event: any, ctx: any) => void) => void;
...
function handleCompactFailed(event: any, ctx: any, runtime: Runtime): void {
	...
	const trace = (ev: string, d?: Record<string, unknown>) => debugLog(ev, d, runtime.config.debugLog === true);
